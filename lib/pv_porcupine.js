...
function integrateWasmJS(integrateCallback) {
	// var wasmTextFile = "pv_porcupine.wast";
	var wasmBinaryFile = "/pv_porcupine.wasm";	// TODO: MAKE THIS A VARIABLE THAT BUMBLEBEE CAN OVERRIDE
	...
	// 			console.log('fetch loaded', wasmBinaryFile);
	// 			debugger;
	//
	// 			if (!response["ok"]) {
	...
	function doNativeWasm(global, env) {  //providedBuffer
		...
		if (!Module["wasmBinary"] &&
			...
			console.log('BumbleBee: fetching wasm', wasmBinaryFile);
			
			const fetchWasm = fetch(wasmBinaryFile, {
			...
			const receive = function (result) {
				...
				// this is now the primary call back back to porcupine.js
				integrateCallback(Module);
			};
			
			WebAssembly.instantiateStreaming(fetchWasm, info).then(receive).catch((function (reason) {
			...
			// todo: in what scenario does this ever take place?
			debugger;
			//instantiateArrayBuffer(receiveInstantiatedSource)
		}
	}
	...
	const loadWasm = function (global, env, providedBuffer) {
		// env = fixImports(env);
		if (!env["table"]) {
		...
		doNativeWasm(global, env, providedBuffer);
	};
	
	loadWasm(Module.asmGlobalArg, Module.asmLibraryArg, buffer);
}
...
var _malloc = Module["_malloc"] = (function () {
	if (!Module["asm"]["_malloc"]) {
		console.error("Fatal Error: no pv_porcupine.wasm loaded?");
		return;
	}
